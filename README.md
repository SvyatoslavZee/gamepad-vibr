...
Start Yandex Browser with DevTools enabled:
...
In another terminal, start the injector:

```bash
node yandex-dualsense-haptics-bridge/inject-devtools-bridge.js
```

Open Igromir / Plus Gaming and connect the controller.

In the page overlay:

1. Click `Connect Controller haptics`.
2. In the WebHID picker choose the real Sony device:
   - `Wireless Controller` for USB.
   - `DualSense Wireless Controller` for Bluetooth.
3. Do not choose `GamePad-1`; that is macOS's synthetic compatibility device.
4. Click `Test`.
5. Confirm that the controller vibrates.
6. Confirm the overlay shows:
   - `bridge yes`
   - `1 gamepad(s)` or more
   - `rumble N`
...
- Package the project as a public GitHub repository and share integration notes
  with Yandex.
